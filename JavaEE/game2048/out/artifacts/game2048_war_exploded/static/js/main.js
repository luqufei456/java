$(function () {
    ...
    function isGameOver() {
        ...
        console.log("当前分数"+$('#gameScore').html());
        console.log("最高分数"+$('#maxScore').html());
        console.log($('#maxScore').html()+$('#gameScore').html());
        // 游戏结束后发送请求更改最高分
        if ($('#gameScore').html() > $('#maxScore').html()) {

            $.ajax({
                url: '/web/IndexServlet',
                //请求类型区分 get post 小型数据 不求安全 用get 否则post
                type: 'POST',
                dataType: 'json',
                data: {'maxScore': $('#gameScore').html()}
            })
            //data是json文件的实例对象 你取什么名字 下面就用什么
                .done(function (data) {
                    console.log(data.saveScore);
                    $('#maxScore').html($('#gameScore').html());
                })
                .fail(function () {
                    alert('连接超时，请检查网络');
                })
